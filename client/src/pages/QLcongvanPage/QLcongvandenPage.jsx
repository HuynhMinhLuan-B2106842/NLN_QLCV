...
const CongvandenPage = ({ setBreadcrumb }) => {
  ...
  const fetchUsers = () => {
    axios.get('http://localhost:5000/api/congvan')
      .then(response => {
        const filteredData = response.data.filter(user => user.danhmuc && user.danhmuc.ten_DM === 'Công văn đến');
  ...
  const handleSearch = (value) => {
    const filtered = users.filter(user =>
        ...
        (user.chude && user.chude.some(chuDe => chuDe.includes(value))) // Tìm kiếm theo chủ đề

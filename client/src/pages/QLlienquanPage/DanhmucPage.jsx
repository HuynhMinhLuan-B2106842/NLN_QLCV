...
const QLDanhMucPage = ({ setBreadcrumb }) => {
    ...
    const handleAddCategory = () => {
        ...
        const topicsArray = newTopics
            ? newTopics.split('\n').map(topic => {
                  const [ten, ...tuKhoa] = topic.trim().split('|');
                  return { ten, tuKhoa: tuKhoa.map(khoa => khoa.trim()) };
              })
            : [];

        axios.post('http://localhost:5000/api/danhmuc', { ten_DM: newCategory, chuDe: topicsArray })
            .then(response => {
                setCategories([...categories, response.data]);
                setNewCategory('');
                setNewTopics('');
    ...
    const showEditModal = (category) => {
        ...
        setEditingCategoryName(category.ten_DM);
        setEditingTopics(
            category.chuDe.map(topic => `${topic.ten}|${topic.tuKhoa.join(',')}`).join('\n')
        );
        setIsModalVisible(true);
    };
    ...
    const handleUpdateCategory = () => {
        ...
        const topicsArray = editingTopics
            ? editingTopics.split('\n').map(topic => {
                  const [ten, ...tuKhoa] = topic.trim().split('|');
                  return { ten, tuKhoa: tuKhoa.map(khoa => khoa.trim()) };
              })
    ...
    return (
        <div className='container'>
            ...
            <div className='mb-3'>
                ...
                />
                <Input.TextArea
                    placeholder='Nhập chủ đề (chủ đề|từ khóa, xuống dòng bằng Enter)'
                    value={newTopics}
                    onChange={(e) => setNewTopics(e.target.value)}
                    rows={4} // Chọn số dòng cho textarea
                />
                ...
                <tbody>
                    {categories.map((category) => (
                        <tr key={category._id}>
                            ...
                            <td>
                                <Button type="primary" onClick={() => showEditModal(category)}>
                                    Chỉnh Sửa
                                </Button>

...
const CongvandiPage = ({ setBreadcrumb }) => {
  ...
  useEffect(() => {
    setBreadcrumb && setBreadcrumb('Công văn đi');
    fetchUsers();
  }, [setBreadcrumb]);
  ...
  const fetchUsers = () => {
    axios.get('http://localhost:5000/api/congvan')
      .then(response => {
        const filteredData = response.data.filter(cv => cv.loaicongvan && cv.loaicongvan.ten_LCV === "Công văn đi");
      ...
      >
        {selectedCongVan && (
          <div>
            ...
            <p><strong>Người liên quan:</strong> {selectedCongVan.nguoilienquan}</p>
